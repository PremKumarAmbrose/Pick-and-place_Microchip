...
int pick_and_place(char Componnt)
{
    ...
    //Tweezer open 5 units wide
        ...
        for(int i = 0; (i<((Angle_diff-Pick_Angle)/3.6)) && !stop; i++){Twister(Rot_dir);}
        ...
        for(int i = 0; (i<((Pick_Angle-Place_Angle)/3.6)) && !stop; i++){Twister(Rot_dir);}
            //Z_axis_and_Tweezer();
            //Tweezer open 5 units wide
